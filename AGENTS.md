...
Single npm package (`@0xpv/Codex-share`) exposing two binaries: `Codex-share` (sharer) and `Codex-connect` (receiver). Source lives in `Codex-share/src/` and `Codex-connect/src/` respectively; both compile into their own `dist/` via the root `package.json`.

## Dev commands

```bash
bun run dev                                      # Codex-share (TUNNEL=0 to skip bore)
bun run dev:connect --share=<url>                # Codex-connect
# or directly:
bun Codex-share/src/index.ts
bun Codex-connect/src/index.ts --share=<url>
```
...
**MITM proxy** (`proxy/mitm.ts`): intercepts TLS only for `INTERCEPT_DOMAINS` (`api.anthropic.com`, `platform.anthropic.com`, `platform.Codex.com`, `mcp-proxy.anthropic.com`). All other CONNECT requests are transparent TCP-piped — never touch the cert or plaintext.
...
**Multi-account** (`proxy/token.ts`): set `CLAUDE_ACCOUNTS=<dir1>,<dir2>,…` (comma-separated `CLAUDE_CONFIG_DIR`s, e.g. `~/.Codex,~/.Codex-acc2`) to expose several accounts; unset = single default account (`~/.Codex`/Keychain), unchanged. Each account's creds come from `<dir>/.credentials.json` (default falls back to the platform store), refreshed via `CLAUDE_CONFIG_DIR=<dir> Codex -p HI`. The receiver picks one at launch (`GET /accounts` → picker → `POST /session/select-account`); the choice is stored per-machine as `machine.selectedAccount` and the MITM injects that account's token. Fixed for the whole session (no mid-session switch).
...
`~/.Codex-share/connections/<machineId>.json` — pruned on startup if `sharedUntil` is past.  
`~/.Codex-share/config.json` — device name.
